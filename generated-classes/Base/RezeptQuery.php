 ...
 *
 *
 * @method     ChildRezeptQuery orderByRezeptId($order = Criteria::ASC) Order by the rezept_id column
 ...
 *
 * @method     ChildRezeptQuery groupByRezeptId() Group by the rezept_id column
 ...
 *
 * @method     ChildRezeptQuery leftJoinnotizen($relationAlias = null) Adds a LEFT JOIN clause to the query using the notizen relation
 * @method     ChildRezeptQuery rightJoinnotizen($relationAlias = null) Adds a RIGHT JOIN clause to the query using the notizen relation
 * @method     ChildRezeptQuery innerJoinnotizen($relationAlias = null) Adds a INNER JOIN clause to the query using the notizen relation
 *
 * @method     \RezeptverwaltungQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 *
 * @method     ChildRezept findOneByRezeptId(int $rezept_id) Return the first ChildRezept filtered by the rezept_id column
 ...
 * @method     ChildRezept[]|ObjectCollection findByRezeptId(int $rezept_id) Return ChildRezept objects filtered by the rezept_id column
...
abstract class RezeptQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'notizen', $modelName = '\\Rezept', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT rezept_id, created_at, updated_at FROM rezept WHERE rezept_id = :p0';
        try {
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        return $this->addUsingAlias(RezeptTableMap::COL_REZEPT_ID, $key, Criteria::EQUAL);
    }
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        return $this->addUsingAlias(RezeptTableMap::COL_REZEPT_ID, $keys, Criteria::IN);
    }

    /**
     * Filter the query on the rezept_id column
     *
     * Example usage:
     * <code>
     * $query->filterByRezeptId(1234); // WHERE rezept_id = 1234
     * $query->filterByRezeptId(array(12, 34)); // WHERE rezept_id IN (12, 34)
     * $query->filterByRezeptId(array('min' => 12)); // WHERE rezept_id > 12
     * </code>
     *
     * @param     mixed $rezeptId The value to use as filter.
     ...
     * @return $this|ChildRezeptQuery The current query, for fluid interface
     */
    public function filterByRezeptId($rezeptId = null, $comparison = null)
    {
        if (is_array($rezeptId)) {
            $useMinMax = false;
            if (isset($rezeptId['min'])) {
                $this->addUsingAlias(RezeptTableMap::COL_REZEPT_ID, $rezeptId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($rezeptId['max'])) {
                $this->addUsingAlias(RezeptTableMap::COL_REZEPT_ID, $rezeptId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(RezeptTableMap::COL_REZEPT_ID, $rezeptId, $comparison);
    }
    ...
    public function filterBynotizen($rezeptverwaltung, $comparison = null)
    {
        if ($rezeptverwaltung instanceof \Rezeptverwaltung) {
            return $this
                ->addUsingAlias(RezeptTableMap::COL_REZEPT_ID, $rezeptverwaltung->getFkrezeptId(), $comparison);
    ...
    {
        if ($rezept) {
            $this->addUsingAlias(RezeptTableMap::COL_REZEPT_ID, $rezept->getRezeptId(), Criteria::NOT_EQUAL);
        }
